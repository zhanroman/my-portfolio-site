(function () {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (tocContainer && tocFab && tocCloseButton && tocLinks.length > 0 && scrollSpySections.length > 0) {

      // --- Логика открытия/закрытия мобильного TOC ---
      const toggleTOC = (forceState) => {
        const isOpen = forceState !== undefined ? forceState : !tocContainer.classList.contains('open');
    ...
    if (lightbox) {
      ...
      const openLightbox = (imageElement) => {
        ...
        const caption = imageElement.closest('.shot')?.querySelector('.shot__cap')?.textContent.trim() ||
                        imageElement.closest('.case-inline-shot')?.querySelector('.case-inline-shot__cap')?.textContent.trim() || '';
